...
runners.forEach((runner) => {
  fullNames.push(runner.first_name)
  fullNames.push(runner.last_name)
})
...
const meanDonation = runners.reduce((total, runner) => {
  return total += runner.donation / runners.length
}, 0)

console.log(meanDonation)

// Percentage of runners requesting small shirts
const smallPercent = runners.filter(runner => runner.shirt_size === "S").length / runners.length

console.log(smallPercent)


// log an array of objects with last names and email sorted alphabetically by last name
const contact = []
runners.forEach((runner) => {
  contact.push({"last_name":runner.last_name, "email":runner.email})
})

contact.sort((a, b) => (a.last_name > b.last_name) ? 1: -1)
console.log(contact)